...
// still to change a couple comments and event names
    ...
    modifier lock() {
        require(slit0.unlocked, 'LOK');
        slit0.unlocked = false;
        _;
        slit0.unlocked = true;
    }
    ...
    modifier onlyFactoryOwner() {
        require(msg.sender == ISwapiiFactory(factoryAddress).holder());
        _;
    }

    constructor() {
        int24 _markSpacing;
        (factoryAddress, tokenA, tokenB, charge, _markSpacing) = ISwapiiPoolDeployer(msg.sender).variables();
        markSpacing = _markSpacing;

        maxLiquidityPerMark = Mark.markSpacingToMaxLiquidityPerMark(_markSpacing);
    }

    /// @dev Common checks for valid tick inputs.
    function checkMarks(int24 tickLower, int24 tickUpper) private pure {
    ...
    function balanceA() private view returns (uint256) {
        (bool success, bytes memory data) =
            tokenA.staticcall(abi.encodeWithSelector(IERC20Minimal.balanceOf.selector, address(this)));
    ...
    function balanceB() private view returns (uint256) {
        (bool success, bytes memory data) =
            tokenB.staticcall(abi.encodeWithSelector(IERC20Minimal.balanceOf.selector, address(this)));
        require(success && data.length >= 32);
        return abi.decode(data, (uint256));
    }

    
    function snapshotTotalInside(int24 tickLower, int24 tickUpper)
        external
        view
        preventDelegateCall
        returns (
    ...
    {
        checkMarks(tickLower, tickUpper);
        ...
        {
            Mark.Detail storage lower = marks[tickLower];
            Mark.Detail storage upper = marks[tickUpper];
        ...
        returns (int56[] memory tickCumulatives, uint160[] memory secondsPerLiquidityCumulativeX128s)
    {
        return
            observations.recognize(
                ...
                secondsAgos,
                slit0.tick,
                slit0.observationIndex,
                liquidity,
                slit0.observationCardinality
            );
    }
    ...
    function increaseRecognizationCardinalityNext(uint16 observationCardinalityNext)
        ...
        preventDelegateCall
    {
        uint16 observationCardinalityNextOld = slit0.observationCardinalityNext; // for the event
        uint16 observationCardinalityNextNew =
            observations.enlarge(observationCardinalityNextOld, observationCardinalityNext);
        slit0.observationCardinalityNext = observationCardinalityNextNew;
    ...
    function configure(uint160 sqrtPriceX96) external {
        require(slit0.sqrtPriceX96 == 0, 'AI');

        int24 tick = TickMath.getTickAtSqrtRatio(sqrtPriceX96);

        (uint16 cardinality, uint16 cardinalityNext) = observations.configure(_blockTimestamp());
    ...
    {
        checkMarks(params.tickLower, params.tickUpper);

        Slit0 memory _slit0 = slit0; // SLOAD for gas optimization
        ...
        if (params.liquidityDelta != 0) {
            if (_slit0.tick < params.tickLower) {
            ...
            } else if (_slit0.tick < params.tickUpper) {
                // current tick is inside the passed range
                uint128 liquidityBefore = liquidity; // SLOAD for gas optimization

                // write an oracle entry
                (slit0.observationIndex, slit0.observationCardinality) = observations.update(
    ...
    function _upgradePosition(
        ...
        spot = spots.obtain(owner, tickLower, tickUpper);
        ...
        if (liquidityDelta != 0) {
            ...
            if (flippedLower) {
                markBitmap.turnOverMark(tickLower, markSpacing);
            }
            if (flippedUpper) {
                markBitmap.turnOverMark(tickUpper, markSpacing);
            }
        }

        (uint256 feeGrowthInside0X128, uint256 feeGrowthInside1X128) =
            marks.obtainFeeGrowthInside(tickLower, tickUpper, tick, _feeGrowthGlobal0X128, _feeGrowthGlobal1X128);
        ...
        if (liquidityDelta < 0) {
            if (flippedLower) {
                marks.remove(tickLower);
            }
            if (flippedUpper) {
                marks.remove(tickUpper);
            }
        }
    }
    ...
    function retrieve(
        ...
        Spot.Detail storage spot = spots.obtain(msg.sender, tickLower, tickUpper);
        ...
        emit Retrieve(msg.sender, recipient, tickLower, tickUpper, amount0, amount1);
    }
    ...
    function burnToken(
        ...
        emit BurnToken(msg.sender, tickLower, tickUpper, amount, amount0, amount1);
    }
    ...
    function swapToken(
        ...
        while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
            ...
            (step.tickNext, step.initialized) = markBitmap.anotherProcessedMarkInAWord(
                state.tick,
                markSpacing,
                zeroForOne
            );
            ...
            if (state.sqrtPriceX96 == step.sqrtPriceNextX96) {
                ...
                if (step.initialized) {
                    ...
                    if (!cache.computedLatestObservation) {
                        (cache.tickCumulative, cache.secondsPerLiquidityCumulativeX128) = observations.recognizeSingle(
        ...
        if (state.tick != slot0Start.tick) {
            ...
            (slit0.sqrtPriceX96, slit0.tick, slit0.observationIndex, slit0.observationCardinality) = (
                state.sqrtPriceX96,
                state.tick,
                observationIndex,
            ...
            slit0.sqrtPriceX96 = state.sqrtPriceX96;
        }
        ...
        if (zeroForOne) {
            if (amount1 < 0) TransferHelper.safeTransfer(tokenB, recipient, uint256(-amount1));
            ...
            if (amount0 < 0) TransferHelper.safeTransfer(tokenA, recipient, uint256(-amount0));
        ...
        emit SwapToken(msg.sender, recipient, amount0, amount1, state.sqrtPriceX96, state.liquidity, state.tick);
        slit0.unlocked = true;
    }
    ...
    function surge(
        ...
        uint256 fee0 = FullMath.mulDivRoundingUp(amount0, charge, 1e6);
        uint256 fee1 = FullMath.mulDivRoundingUp(amount1, charge, 1e6);
        uint256 balance0Before = balanceA();
        uint256 balance1Before = balanceB();

        if (amount0 > 0) TransferHelper.safeTransfer(tokenA, recipient, amount0);
        if (amount1 > 0) TransferHelper.safeTransfer(tokenB, recipient, amount1);
        ...
        if (paid0 > 0) {
            uint8 feeProtocol0 = slit0.feeProtocol % 16;
            uint256 fees0 = feeProtocol0 == 0 ? 0 : paid0 / feeProtocol0;
            if (uint128(fees0) > 0) protocolFees.token0 += uint128(fees0);
            feeGrowthGlobal0X128 += FullMath.mulDiv(paid0 - fees0, FixedPoint128.Q128, _liquidity);
        }
        if (paid1 > 0) {
            uint8 feeProtocol1 = slit0.feeProtocol >> 4;
    ...
    function setProtocolCharge(uint8 feeProtocol0, uint8 feeProtocol1) external lock onlyFactoryOwner {
        ...
        uint8 feeProtocolOld = slit0.feeProtocol;
        slit0.feeProtocol = feeProtocol0 + (feeProtocol1 << 4);
